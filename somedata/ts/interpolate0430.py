...
for file in files:
	if '-1' in file:
		...
		# comupute average of -2 below350
		av2=0
		i2=1
		sum=0
		count=0
		while ws2['a%d' % i2].value:
			if type(ws2['a%d' % i2].value) !=float:
				i2=i2+1
				continue	
			if ws2['a%d' % i2].value<350:
				sum=sum+ws2['b%d' % i2].value
				count=count+1
				i2=i2+1
			else:
				break
		...
		while ws1['a%d' % i].value:
			...
			if ws1['a%d' % i].value<350:
				rlist.append([v1-v2,yy[i-7],yyc[i-7]])
			else:
				rlist.append([v1-vms-av2,yy[i-7],yyc[i-7]])
			i=i+1
		for t in rlist:
			wsresult.append(t)
